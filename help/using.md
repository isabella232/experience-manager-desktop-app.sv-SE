...
* **[!UICONTROL Open In Web]** åtgärd: Om du vill visa resursen i AEM webbgränssnitt öppnar du den på webben. Du kan initiera fler arbetsflöden från AEM gränssnitt, som att uppdatera metadata eller resursidentifiering.

* **[!UICONTROL Delete]** åtgärd: Ta bort resursen från AEM DAM-databas. Åtgärden tar bort den ursprungliga kopian av resursen på AEM. Om du bara vill ignorera ändringar i den lokala resursen läser du [Ignorera ändringar](using.md#edit-assets-upload-updated-assets).

* **[!UICONTROL Upload Changes]**: Skrivbordsappen överför bara den uppdaterade resursen när du uttryckligen överför den till AEM. När du sparar redigeringarna sparas ändringarna bara på den lokala datorn. När du överför en resurs checkas den automatiskt in och låsikonen tas bort. Se [Redigera resurser](using.md#edit-assets-upload-updated-assets).

## Aktivera skrivbordsåtgärder i AEM webbgränssnitt {#desktopactions-v2}
...
Du kan bläddra till, söka efter och förhandsvisa de resurser som finns i AEM, allt inifrån skrivbordsprogrammet. Prova följande i appen:
...
1. Sök med nyckelord för att hitta en relaterad resurs i AEM. Använd `?` och `*` som jokertecken. Dessa jokertecken ersätter ett enda tecken eller flera tecken. Filtrera och sortera resultatet efter behov.
...
Du kan hämta resurserna på det lokala filsystemet. Programmet hämtar resurserna från AEM server och sparar samma kopia i det lokala filsystemet.
...
Om en eller flera mappar har valts och hämtats hämtar programmet bara resurser som lagras direkt i mappen/mapparna i AEM. Det hämtar inte resurser från undermappar automatiskt.
...
>[!NOTE]
>
>För bakåtkompatibilitet med AEM datorprogram v1.x hanteras de filer som visas från en lokal nätverksresurs, endast lokalt tillgängliga filer visas. Skrivbordssökvägarna för de visade filerna är samma som sökvägarna som skapas i programmet v1.x.
...
Öppna resurser för redigering när du vill göra ändringar och överföra de uppdaterade resurserna till AEM server. Om du vill undvika konflikter med redigeringar av andra användare använder du programmet för att starta en redigeringssession. Innan du börjar redigera bör du kontrollera att resursen inte har någon låsikon, det vill säga att en annan användare inte redigerar resursen.
...
>[!NOTE]
>
>Om filerna inte kan överföras och du ansluter till AEM 6.5.1 eller senare ska du läsa den här [felsökningsinformationen](troubleshoot.md#upload-fails).
...
![Ladda upp flera lokala mappar från skrivbordet i](assets/upload_local_folders_da2.png "AEMBulk ladda upp flera lokala mappar från skrivbordet till AEM")
...
Se [Överför resurser till AEM](#upload-and-add-new-assets-to-aem)om du vill visa en lista över resurser som överförts under en viss session.

## Avancerat arbetsflöde: starta från AEM Assets webbgränssnitt {#adv-workflow-start-from-aem-ui}

Starta vid behov arbetsflödet från AEM Assets webbgränssnitt. Skrivbordsappen är integrerad med AEM att ta över när det efterfrågas med skrivbordsåtgärder.
...
![Skrivbordsåtgärder i AEM](assets/assets_web_actions_da2.png "webbgränssnittSkrivbordsåtgärder i AEM webbgränssnitt")
...
Om en lokalt hämtad resurs uppdateras på AEM server visas programmets **[!UICONTROL Modified remotely]** status. Du kan antingen ta bort din lokala kopia eller uppdatera den lokala kopian genom att klicka på [!UICONTROL Remove] respektive [!UICONTROL Update] . Med länkar i dialogrutan kan du visa båda versionerna av resursen.
...
När du använder AEM datorprogram för att öppna filer med länkade resurser hämtas resurserna i förväg och visas i de ursprungliga programmen. För att det här arbetsflödet ska fungera måste ditt inbyggda program ha stöd för placering av länkar till lokala resurser och AEM måste ha stöd för att lösa länkarna i de binära filerna till serversidans referenser.

AEM datorprogram har stöd för det här arbetsflödet med några utvalda Adobe Creative Cloud-program och -filformat - Adobe InDesign, Adobe Illustrator och Adobe Photoshop. Med arbetsflödet kan du arbeta effektivt med de Creative Cloud-filer som stöds. Om användare A placerar ett fåtal resurser i en InDesign-fil och checkar in den i AEM, ser användare B resurserna i InDesign-filen även om resurserna inte är en del av filen. Resurserna hämtas lokalt till dator med användare B.
...
1. Håll en INDD-fil med monterade resurser i AEM. Mer information om hur du skapar en sådan INDD-fil finns i [Placera grafik](https://helpx.adobe.com/indesign/using/placing-graphics.html).
1. I skrivbordsappen finns INDD- **[!UICONTROL Edit]** filen med monterade resurser i AEM.
...
I många scenarier hämtas resurserna från AEM server lokalt till filsystemet. Nedladdningarna förbrukar bandbredd och diskutrymme. Genom att känna till scenarierna kan du optimera väntetiden för nedladdningen.
...
Om programmet är installerat och tillåts att göra det, slutförs åtgärderna när du använder det [!UICONTROL Desktop Actions] från AEM webbgränssnitt. Programmet hämtar resursen först och slutför sedan åtgärden.
